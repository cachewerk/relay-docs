...
| `relay.max_endpoint_dbs`          | `1`              | The maximum number of in-memory caches Relay allocates per connection endpoint (distinct Redis connections). PHP workers are distributed across these caches, e.g. connecting to two separate instances will double the number of caches. The default of `1` is right for most deployments. See [Performance](/docs/1.x/performance). |
