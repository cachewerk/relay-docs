...
| `relay.cluster.read_timeout`      |                  | The maximum number of seconds to send request to appropriate cluster node and read response from it |
| `relay.session.compression`       | `none`           | Compression algorithm used for session data. Supported values; `lzf`, `lz4`, `zstd` and `none` (or empty string) |
| `relay.session.compression_level` |                  | Different compression algorithms provided different levels for tuning performance and data size |
| `relay.session.locking_enabled`   | `0`              | Whether enable or not session locking to avoid race conditions and keep session data consistent across requests |
